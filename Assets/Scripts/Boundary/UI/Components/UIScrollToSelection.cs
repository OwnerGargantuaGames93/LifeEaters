...
namespace Boundary.UI.Components
{
    ...
    public class UIScrollToSelection : MonoBehaviour
    {
        ...
        private void UpdateVerticalScrollPosition(RectTransform selection)
        {
            ...
            // Prevent oscillation at the top
            if (TargetScrollRect.verticalNormalizedPosition >= 1f && offlimitsValue > 0)
                return;
            
            // Prevent oscillation at the bottom
            if (TargetScrollRect.verticalNormalizedPosition <= 0f && offlimitsValue < 0)
                return;
